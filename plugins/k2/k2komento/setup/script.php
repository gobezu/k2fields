...
class plgK2komentoInstallerScript {
	...
	public function install($adapter, $isUpdate = false) {
                $komento = true;
                if (JFolder::exists(JPATH_SITE.'/components/com_komento/themes/k2fields')) {
                        $source = $adapter->get('parent')->getPath('source').'/setup/komento_plugin.php';

                        if (JFile::exists($source))
                                JFile::copy($source, JPATH_SITE.'/components/com_k2/komento_plugin.php');

                        $source = $adapter->get('parent')->getPath('source').'/setup/theme';
                        
                        if (JFolder::exists($source)) {
                                JFolder::copy($source, JPATH_SITE.'/components/com_komento/themes/k2fields');
                        }
                } else {
                        $komento = false;
                }
                ...
                if (!$isUpdate) {
                        ...
                        if (JFile::exists(dirname(__FILE__).'/sp.install.mysqli.sql')) {
                                if (class_exists('PDO')) {
...
<table class="adminlist">
	...
	<tbody>
		...
		<?php endif; ?>
			...
			<tr>
                                <td colspan="2">
                                        <div class="error"><?php echo JText::_('Komento/Rate integration not installed since Komento was not installed. If you want to use it please install Komento first and then reinstall k2fields now or at later time please make sure to copy the theme folder from plugins/k2/k2komento/setup/theme to components/com_komento/themes/ and rename the folder to k2fields and make sure to select k2fields as Komento theme. Also copy plugins/k2/k2komento/setup/komento_plugin.php to components/com_k2/komento_plugin.php.'); ?></div>
                                </td>
			</tr>
		<?php else: ?>
			<tr>
                                <td colspan="2">
                                        <div class="info"><?php echo JText::_('Komento/Rate integration installed. Please make sure to select k2fields as theme for Komento'); ?></div>
                                </td>
			</tr>
                <?php endif; ?>
                <?php if (!empty($aerror)) : ?>
