...
var SliRate = new Class({
        ...
        createRate: function() {
                var 
                        frm = $$('#comments form')[0],
                        itemid, itemidName = 'article_id',
                        key, lbl, opts, criteria, i, n, sel, selc, opt, ratec, id, ui, width, criterias = [], rEls = [], rEl;
                        
                itemid = frm.getElements('input[name='+itemidName+']')[0].value;
                ratec = new Element('div', {'class':'ratescontainer'});
                ratec.inject(frm, 'top');
                
                new Element('input', {'type':'hidden','name':'k2fieldsrate','value':1}).inject(ratec);
