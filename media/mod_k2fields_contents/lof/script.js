...
var LofK2SlideShowsCreator = new Class( {
        initialize:function(eMainIds, lofOptions, options){
                var el, lof, eMain, eNavigator, eNavOuter, btnCont, preload;
                for (var i = 0, n = eMainIds.length; i < n; i++) {
                        eMain = eNavigator = eNavOuter = null;
                        el = document.id(eMainIds[i]);
                        el = el.getChildren('.lofass-container')[0];
                        eMain = el.getChildren('.lof-main-wapper')[0];
                        eNavOuter = el.getChildren('.lof-navigator-outer');
                        if (eNavOuter.length) {
                                eNavOuter = eNavOuter[0];
                                eNavigator = eNavOuter.getElement('.lof-navigator');
                        }
                        ...
                        if (options.displayButton) {
                                btnCont = el.getChildren('.lof-buttons-control');
                                if (btnCont.length) {
                                        btnCont = btnCont[0];
                                        lof.registerButtonsControl( 
                                                'click', 
                                                {
                                                        next:btnCont.getElement('.lof-next'),
                                                        previous:btnCont.getElement('.lof-previous')
                                                } 
                                        );
                                }
                        }
                        preload = el.getChildren('.preload');
                        if (preload.length) preload = preload[0];
                        else preload = null;
                        lof.start(options.autoStart, preload);
                }
        }
});
...
var LofK2SlideShow = new Class( {
        initialize:function( eMain, eNavigator,eNavOuter, options ){
                ...
                this.onClick = false;
                if( eMain != null ){
                        this.slides    = eMain.getChildren( this.setting.mainItemSelector );
                        this.maxWidth  = eMain.getStyle('width').toInt();
                        this.maxHeight = eMain.getStyle('height').toInt();
                        this.styleMode = this.__getStyleMode();  
                        var fx =  Object.append({
                                waiting:false, 
                                onComplete:function(){
                                        this.onClick=false
                                        }.bind(this)
                ...
                if( eNavigator != null && eNavOuter != null ){
                        ...
                        this.navigatorFx = new Fx.Tween( eNavigator,{
        ...
        preloadImages:function( obj ){  
                var counter=0, _length, image, self = this, imgs = this.slides.getElements('img');
                imgs = imgs.flatten();
                _length = imgs.length;
