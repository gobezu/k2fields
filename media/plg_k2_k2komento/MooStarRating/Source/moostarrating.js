...
var MooStarRating = new Class({
	...
	initialize: function(options) {
		...
		this.options.form = document.id(formQuery);
		...
		$$(this.options.selector).each(function (item) {
			...
			if (this.options.tip) {
				...
				title = title.replace('[COUNT]', count);
                                title = title.replace('[CRITERIA]', this.options.criterias ? this.options.criterias[i] : '');
	...
	fillStars: function (hoverIndex) {
		$$(this.stars).each(function (star, index) {
                        var pos = '0 0';
			if (hoverIndex == null) if (index <= this.currentIndex) pos = '0 -16px';
			if (hoverIndex != null) if (index <= hoverIndex) pos = '0 -16px';
                        star.setStyle('background-position', pos);
...
//			star.setStyle('background-image', 'url("' + this.options.imageFolder + image + '")');
	...
	starEnter: function (index) {
		...
		if (this.options.tip && this.options.tipTarget) 
			document.id(this.options.tipTarget).set(
                                this.options.tipTargetType, 
                                this.stars[index].retrieve('ratingTip')
                        );
	...
	starLeave: function () {
		...
		this.fillStars();
		if (this.options.tip && this.options.tipTarget) 
			document.id(this.options.tipTarget).set(this.options.tipTargetType, '');
