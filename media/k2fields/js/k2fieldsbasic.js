...
var k2fields_type_basic = {
        ...
        createDays: function(holder, proxyField, value, condition) {
                ...
                if (this.isMode('search')) {
                        ...
                        if (isDsD) {
                                ...
                                var _valid = this.getOpt(proxyField, 'valid');
                                this.setOpt(proxyField, 'valid', 'date');
                                if (typeOf(value) == 'array') value = value[0];
                                value = value+'%%%%%%';

                                var result = this.createFieldSub(proxyField, value, condition, holder);
                                this.setOpt(proxyField, 'valid', _valid);
                                return result;
                        }
                }
                ...
                var result = this.createBasic(holder, proxyField, value, condition);
                this.setOpt(proxyField, 'valid', _valid);
                return result;
        },
        ...
        __createSlider: function(holder, proxyField, value, condition) {
                ...
                this.setOpts(proxyField, def, false);

                defaultValue = this.getDefaultValue(proxyField);
                ...
                if (value == defaultValue && this.isMode('search')) document.id(val).set('value', '');
        ...
        chosenize: function(el, name, isMultiple, opts) {
                if (!opts) opts = {};

                opts['name'] = name;
                opts['selectchosen'] = 1;

                if (isMultiple) opts['multiple'] = 1;
        ...
        __createChosen:function(el, proxyField, position) {
                ...
                var options = el.getElements('option'), i, n = options.length;

                for (i = 0; i < n; i++) {
                        if (!options[i].get('value')) {
                                el.getElements('option:nth-child('+(i+1)+')').set('html', '');
                                break;
                        }
                }
                ...
                if (val = this.getOpt(proxyField, 'chosen.templateselected')) {
                        el.set('data-templateSelected', val);
                ...
                jQuery(el).chosen(opts).change(function() {
        ...
        _orderValues:function(proxyField) {
                ...
                if (typeOf(values[0]) != 'object') {
                        ...
                        if (values[0][ord]) values = values.sortBy(ord);
                }

                if (this.chkOpt(proxyField, 'valuesorder', 'reverse')) {
                        var nValues = [];
                        for (var i = 0, n = values.length; i < n; i++) {
                                nValues[n-i-1] = values[i];
                        }
                        values = nValues;
                }
...
//                this.setOpt(field, 'depson', depson);
        ...
        initiateFieldDependency: function(field, forSybs) {
                ...
                if (deps) {
                        for (k in deps) {
                                if (typeof deps[k] == 'function') continue;
