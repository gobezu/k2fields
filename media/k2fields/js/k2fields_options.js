...
var k2fieldsoptions = new Class({
        ...
        initializeOptions: function(opts) {
                ...
                if (opts['subfields']) {
                        for (var i = 0; i < opts['subfields'].length; i++) {
                                for (var optName in opts['subfields'][i]) {
                                        if (re.test(optName)) {
                                                newOptName = optName.replace(re, '');
                                                opts['subfields'][i][newOptName] = opts['subfields'][i][optName];
                                        }
                                }
                        }
                }
        ...
        parseDateNotations: function(value, format) {
                if (typeof value == 'string') {
                ...
                return value.format(format || this.options.datetimeFormat);
        }
});
