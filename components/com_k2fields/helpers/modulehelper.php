...
';
	...
	static function _getList($params, $componentParams, $format = 'html', $partBy = 'category') {
                ...
                $isMap = false;
                if ($params->get('template') == 'map' && $params->get('mapfield')) {
                        $isMap = true;
                        $query .= ', v.lat, v.lng, v.txt';
                }
                ...
                $queryFrom = " FROM (SELECT * FROM #__k2_items AS ii ".$innerQueryWhere.") AS i ";
                // Currently map captures only the first IP
                // TODO: fetch all listindex and aggregate in module over item id
                // TODO: same as above goes for conditional comments which can be aggregated in SQL
                if ($isMap) $queryFrom .= 'INNER JOIN #__k2_extra_fields_values v ON i.id = v.itemid AND v.fieldid = '.(int) $params->get('mapfield').' AND v.listindex = 0 AND v.partindex = 0 ';
                $queryFrom .= ($allowEmpty ? 'RIGHT' : 'LEFT')." JOIN #__k2_categories AS c ON c.id = i.catid";
                ...
                if (empty($items)) {
                        if (!empty($limitPerCat) || empty($limit)) {
                                if (!$isMap && empty($limitPerCat)) $limitPerCat = 5;

                                if (!empty($limitPerCat)) $cnts = array_fill(0, count($categories), $limitPerCat);
                                else $cnts = false;
                        } else {
                                $categories = array($categories);
                                $cnts = array($limit);
                        ...
                        foreach ($categories as $c => $cat) {
                                ...
                                $queries[] = "(" . $q . " AND c.id IN ({$sql}) ORDER BY " . $orderby . ($cnts ? " LIMIT 0, ".$cnts[$c] : '') . ")";
                        }
        ...
        public static function prepareListMap($items, $params, $componentParams, $format) {
                $items = JprovenUtility::flatten($items);
                foreach ($items as $item) {
                        if (!$item->id) continue;
		...
		foreach ($items as $catId => &$itemsPerCat) {
                        foreach ($itemsPerCat as &$item) {
                                ...
                                } else if ($params->get('itemImage') == 'k2') {
