...
class K2FieldsHelperRoute {
	function getItemRoute($id, $catid = 0) {
        ...
        public static function createCategoryLink($category, $searchQueries = array(), $title = '', $mode = '', $class = 'k2fcatlink') {
                ...
                if ($task == 'category') {
                        ...
                        if ($item = self::_findItem($needles)) $link .= '&Itemid='.$item->id;
                } else {
                        $item = '';
                ...
                foreach ($searchQueries as $key => $val) $link .= '&'.$key.'='.$val;
        ...
        public static function createItemLink($item, $title = '', $mode = '', $class = 'k2flink') {
                ...
                if (isset($item->k2item) && $item->k2item) $link .= '&k2item=1';
                if (isset($item->k2cat) && $item->k2cat) $link .= '&k2cat='.$item->k2cat;
	...
	function getTagRoute($tag) {
		...
		$match = null;
                ...
                if (!isset($needles['itemlist'])) return $match;
...
//                        
//                        if (isset($needles[0])) jdbg::pe($needles);
//                        $needles = array('itemlist'=>$needles[0]['cid']);
                }
		...
		foreach ($needles as $needle=>$id) {
			...
			if (!is_null($match)) {
                ...
                if (is_null($match) && $option == 'com_k2') $match = self::_findItem($needles, 'com_k2fields');
		return $match;
	}
}

class K2HelperRoute extends K2FieldsHelperRoute { }